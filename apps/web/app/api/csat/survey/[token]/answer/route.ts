...
export async function POST(req: NextRequest, { params }: any) {
  ...
  let url = `${apiBaseUrl()}/csat/survey/[token]/answer`;
  for (const [k, v] of Object.entries(p)) url = url.replace(`[${k}]`, v as string);
  const res = await fetch(url, { method: 'POST', headers: { authorization: `Bearer ${session.accessToken}`, 'content-type': 'application/json' }, body: JSON.stringify(omitTenantIdFromBody(body)), cache: 'no-store' });

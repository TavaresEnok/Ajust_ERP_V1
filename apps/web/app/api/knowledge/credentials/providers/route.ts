...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(
    `${apiBaseUrl()}/knowledge/credentials/providers?${params.toString()}`,
    {
      method: 'GET',
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: text || 'Falha ao listar provedores de credenciais.' },
      { status: response.status },
    );
  }

  const proxied = NextResponse.json(
    text ? JSON.parse(text) : { items: [], total: 0, limit: 0, offset: 0 },
  );

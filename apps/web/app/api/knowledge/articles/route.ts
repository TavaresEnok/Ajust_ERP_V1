...
export async function GET(request: NextRequest) {
  ...
  if (!response.ok) {
    return NextResponse.json({ error: text || 'Falha ao listar artigos.' }, { status: response.status });
  }
...
export async function POST(request: NextRequest) {
  ...
  const text = await response.text();
  if (!response.ok) {
    return NextResponse.json({ error: text || 'Falha ao criar artigo.' }, { status: response.status });
  }

...
export async function PATCH(request: NextRequest, context: { params: Promise<{ tenantId: string }> }) {
  ...
  const response = await fetch(`${apiBaseUrl()}/iam/tenants/${encodeURIComponent(tenantId)}`, {
    method: 'PATCH',
    headers: {

...
export async function PATCH(
  ...
  const response = await fetch(`${apiBaseUrl()}/iam/tenants/${encodeURIComponent(tenantId)}/sectors/${encodeURIComponent(sectorId)}`, {
    method: 'PATCH',
    headers: {
  ...
  if (!response.ok) {
    return NextResponse.json({ error: text || 'Falha ao atualizar setor.' }, { status: response.status });
  }
...
export async function DELETE(
  ...
  const text = await response.text();
  if (!response.ok) {
    return NextResponse.json({ error: text || 'Falha ao remover setor.' }, { status: response.status });
  }

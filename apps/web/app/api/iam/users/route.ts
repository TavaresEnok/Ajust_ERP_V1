...
export async function GET(request: NextRequest) {
  ...
  const requestedTenantId = rawParams.get('tenantId');
  const params = omitTenantIdFromSearchParams(rawParams);
  if (requestedTenantId) params.set('targetTenantId', requestedTenantId);
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: text || 'Falha ao listar usuarios.' },
...
export async function POST(request: NextRequest) {
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: text || 'Falha ao criar usuario.' },
      { status: response.status },
    );
  }

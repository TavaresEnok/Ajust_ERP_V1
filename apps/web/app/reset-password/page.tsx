...
function ResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || 'Falha ao redefinir senha. O token pode estar expirado.');
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0f1c] text-white p-4">
      <div className="max-w-md w-full bg-[#111b2e] p-8 rounded-2xl shadow-xl border border-slate-800">
        <div className="flex justify-center mb-6">
          <Image src="/images/ajust-logo-dark.png" alt="Ajust ERP" width={150} height={50} className="opacity-90" />
        </div>
        ...
        {!token ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {message && <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 text-sm">{message}</div>}

...
type GlobalOrdersHelpers = {
  ...
  filterOrdersByWindow: (orders: OrderRow[], window: unknown, field: string) => OrderRow[];
  isActive: (order: OrderRow) => boolean;
  isClosed: (order: OrderRow) => boolean;
  STATUSES: string[];
  PRIORITIES: string[];
  getDelayHours: (order: OrderRow) => number;
  isOverdueActive: (order: OrderRow) => boolean;
...
type GlobalOrdersViewModuleProps = {
  orders: OrderRow[];
  onSelectOS: (order: OrderRow) => void;
  title?: string;
  showTenantFilter?: boolean;
  helpers: GlobalOrdersHelpers;
};
...
export function GlobalOrdersViewModule({
  ...
  const [customRange, setCustomRange] = useState({
    start: toInputDate(Date.now() - 7 * DAY_MS),
  ...
  const providerOptions = useMemo(
    () => ['Todos', ...Array.from(new Set(orders.map((o) => o.provider))).sort()],
    [orders],
  );
  const techOptions = useMemo(
    () => [
      'Todos',
      'Sem tecnico',
      ...Array.from(new Set(orders.map((o) => o.tech).filter(Boolean))).sort(),
    ],
    [orders],
  );
  const typeOptions = useMemo(
    () => ['Todos', ...Array.from(new Set(orders.map((o) => o.type))).sort()],
    [orders],
  );
  const sgpOptions = useMemo(
    () => ['Todos', ...Array.from(new Set(orders.map((o) => o.sgpStatus))).sort()],
    [orders],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return scoped.filter((o: OrderRow) => {
      const matchSearch =
        !q ||
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-end md:items-start gap-4">
        <div>
          ...
          <p className={cn('text-sm', dark ? 'text-slate-400' : 'text-slate-500')}>
            Lista mestra de chamados com filtros avancados.
          </p>
        </div>
        <WindowControls
          period={period}
      ...
      <div
        className={cn(
          'p-4 rounded-xl border shadow-sm grid grid-cols-1 md:grid-cols-12 gap-3',
          dark
      ...
      >
        <div className="md:col-span-4 relative">
          ...
          <input
            ...
            className={cn(
              'w-full pl-10 pr-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500',
              dark
                ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200 placeholder:text-slate-500'
                : 'border-slate-200',
            )}
          />
        </div>
        ...
        <div className="md:col-span-2">
          <select
            ...
            className={cn(
              ...
              dark ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200' : 'border-slate-200',
            )}
          >
            <option>Todas</option>
        ...
        <div className="md:col-span-2">
          <select
            value={sgpFilter}
            onChange={(e) => setSgpFilter(e.target.value)}
            className={cn(
              'w-full px-3 py-2 border rounded-lg text-sm',
              dark ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200' : 'border-slate-200',
            )}
        ...
        {showTenantFilter && (
          <div
            className={cn(
              'md:col-span-4 flex items-center text-xs',
      ...
      >
          ...
          <table className="w-full text-sm">
            <thead
              className={cn(
                'border-b',
                dark
                  ? 'bg-[#0d1628]/95 text-slate-400 border-slate-700/60'
            ...
            >
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs uppercase font-mono">Legado</th>
            ...
            <tbody className={cn('divide-y', dark ? 'divide-slate-700/50' : 'divide-slate-100')}>
              {rows.map((o: OrderRow) => {
                ...
                return (
                  <tr
                    key={o.id}
                    className={cn(dark ? 'hover:bg-white/[0.03]' : 'hover:bg-slate-50/80')}
                  >
                    <td
                      className={cn(
                        'px-6 py-3 font-mono font-medium',
                        dark ? 'text-slate-200' : 'text-slate-700',
                      )}
                    >
                      {o.protocol}
                    </td>
                    <td className={cn('px-6 py-3', dark ? 'text-slate-200' : 'text-slate-700')}>
                      {o.provider}
                    </td>
                    <td className={cn('px-6 py-3', dark ? 'text-slate-400' : 'text-slate-600')}>
                      {o.tech || '-'}
                    </td>
                    ...
                    <td className="px-6 py-3">
                      <Badge color={getPriorityColor(o.priority)} dark={dark}>
                        {o.priority}
                      </Badge>
                    </td>
                    <td className="px-6 py-3">
                      <Badge color={getStatusColor(o.status)} dark={dark}>
                        {o.status}
                      </Badge>
                    </td>
                    <td
                    ...
                    <td
                      className={cn(
                        'px-6 py-3 text-right font-mono text-xs',
                        isOverdueActive(o)
                    ...
                    <td
                      className={cn(
                        'px-6 py-3 text-right font-mono text-xs',
                        parseFloat(delay) > 0 ? 'text-rose-600 font-bold' : 'text-slate-400',
                      )}
                    >
                      {delay}
                    </td>
        ...
        <div
          className={cn(
            ...
            dark ? 'bg-[#0d1628]/80 border-slate-700/60' : 'bg-slate-50 border-slate-200',
          )}
        >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                dark
                  ? 'bg-[#0c1528] border-slate-700/60 text-slate-300 hover:bg-white/[0.04]'

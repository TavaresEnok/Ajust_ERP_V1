...
type SlaPriority = 'BAIXA' | 'NORMAL' | 'ALTA' | 'CRITICA';
...
export function SlaBuilderView({
  dark,
  tenantId,
  onToast,
}: {
  ...
  const [form, setForm] = useState<{
    ...
    hours: string;
    isOverride: boolean;
  }>({ priority: 'NORMAL', serviceOrderType: '', hours: '4', isOverride: false });
  ...
  const load = async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch(`/api/sla-policies?tenantId=${encodeURIComponent(tenantId)}`, {
        cache: 'no-store',
      });
      ...
      if (res.ok) {
        ...
        const message =
          ...
          typeof (payload as { error?: unknown }).error === 'string'
            ...
            : 'Falha ao carregar políticas de SLA.';
        setLoadError(message);
        setPolicies([]);
      }
    } catch {
      const message = 'Falha ao carregar políticas de SLA.';
      setLoadError(message);
      onToast(message);
      setPolicies([]);
    } finally {
  ...
  const save = async () => {
    if (!form.priority || !form.hours) {
      onToast('Prioridade e prazo são obrigatórios.');
      return;
    }
    setSaving(true);
    try {
      ...
      onToast(e instanceof Error ? e.message : 'Falha ao criar política de SLA.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/sla-policies/${id}?tenantId=${tenantId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Falha ao remover regra de SLA.');
      onToast('Regra removida.');
      load();
    } catch (e: unknown) {
      onToast(e instanceof Error ? e.message : 'Falha ao remover regra de SLA.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={cn('text-sm mt-0.5', dark ? 'text-slate-400' : 'text-slate-500')}>
            Crie regras visuais de tempo máximo de atendimento (SLA) para a sua operação.
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold shadow-lg shadow-indigo-500/20 transition-colors"
        >
      ...
      <div
        className={cn(
          ...
          dark ? 'bg-slate-800/30 border-slate-700/50' : 'bg-indigo-50 border-indigo-200',
        )}
      >
        ...
        <div>
          ...
          <p className={cn('text-xs leading-relaxed', dark ? 'text-slate-400' : 'text-slate-600')}>
            Por padrão, o SLA é calculado com base na <strong>Prioridade</strong> da O.S. Se você
            criar uma regra marcando <strong>Exceção de Tipo (Override)</strong>, essa regra terá
            peso maior e vai sobrepor a prioridade, baseada no Tipo de Serviço (ex: Rompimento).
          </p>
        </div>
      </div>
      {loadError && (
      ...
      >
        <table className="w-full text-sm">
          <thead>
            <tr
              className={cn(
                'border-b text-xs uppercase font-bold tracking-wider',
                dark
              ...
              policies.map((p) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 text-center">
                    {p.isOverride ? (
                      <span className="px-2 py-1 rounded bg-amber-500/10 text-amber-500 text-[10px] font-bold">
                        SIM
                      </span>
                    ) : (
                  ...
                  <td className="px-5 py-4 text-center">
                    {p.active ? (
                      ...
                      <span className="px-2 py-1 rounded bg-rose-500/10 text-rose-500 text-[10px] font-bold">
                        INATIVO
                      </span>
                    )}
                  </td>
          ...
          >
            ...
            <div className="p-6 space-y-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...
                      serviceOrderType: e.target.checked ? 'INSTALACAO' : '',
                    })
                  }
              ...
              {!form.isOverride ? (
                <div>
                  ...
                  <select
                    value={form.priority}
                    onChange={(e) => setForm({ ...form, priority: e.target.value as SlaPriority })}
                ...
                <div>
                  ...
                  <select
                    value={form.serviceOrderType}
                    onChange={(e) => setForm({ ...form, serviceOrderType: e.target.value })}
                    className={cn(inputCls, 'cursor-pointer')}
                  >
            ...
            >
              ...
              <button
                onClick={save}
                disabled={saving}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold disabled:opacity-60 flex items-center gap-2"
              >

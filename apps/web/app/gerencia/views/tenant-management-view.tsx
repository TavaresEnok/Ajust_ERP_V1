...
type TenantRole = { code: string; name: string };

type TenantHelpers = {
  useTheme: () => { dark: boolean };
  Search: React.ComponentType<{ className?: string; size?: number }>;
  RefreshCw: React.ComponentType<{ size?: number }>;
  Plus: React.ComponentType<{ size?: number }>;
  Badge: React.ComponentType<{ color: string; children?: React.ReactNode }>;
  ...
  isActive: (order: OrderSummary) => boolean;
...
type TenantManagementViewModuleProps = {
  ...
  helpers: TenantHelpers;
};

export function TenantManagementViewModule({
  tenants,
  orders,
  notify,
  onReloadTenants,
  helpers,
}: TenantManagementViewModuleProps) {
  const {
    useTheme,
    Search,
    RefreshCw,
  ...
  const [userTenant, setUserTenant] = useState<TenantRecord | null>(null);
    ...
    const loadRoles = async () => {
      try {
        ...
        const mapped = payload
          .filter(
            (role: unknown): role is { code: string; name?: string } =>
              Boolean(role) &&
              typeof role === 'object' &&
              typeof (role as { code?: unknown }).code === 'string',
          )
          .map((role) => ({
            code: role.code,
            name: role.name || ROLE_CODE_TO_LABEL[role.code] || role.code,
          }));
        if (mapped.length > 0) {
  ...
  const fetchTenantUsers = async (targetTenantId: string): Promise<TenantUser[]> => {
    ...
    const response = await fetch(`/api/iam/users?tenantId=${encodeURIComponent(targetTenantId)}`, {
      cache: 'no-store',
    });
    if (!response.ok) {
  ...
  const enriched = useMemo(
    () =>
      tenants.map((t: TenantRecord) => {
        const own = orders.filter((o: OrderSummary) => o.provider === t.name);
  ...
  const stats = useMemo(() => {
    ...
    const activeOrdersTotal = enriched.reduce((acc: number, t) => acc + t.activeOrders, 0);
  ...
  const filtered = useMemo(() => {
    ...
    let list = [...enriched].filter(
      (t) =>
        ...
        (t.cpfCnpj || '').toLowerCase().includes(q),
    );
  ...
  const createTenant = () => {
    if (busy) return;
    if (
      !newTenant.tradeName.trim() ||
      !newTenant.taxId.trim() ||
      !newTenant.techContactEmail.trim()
    ) {
  ...
  const refreshTenants = () => {
    ...
    (async () => {
      ...
      try {
        await onReloadTenants?.();
        notify?.('Lista de clientes atualizada.');
      } catch (error) {
        notify?.(error instanceof Error ? error.message : 'Falha ao atualizar tenants.');
      } finally {
        setBusy(false);
      }
    })();
  ...
  const openTenantUsers = (tenant: TenantRecord) => {
    setUserTenant(tenant);
    fetchTenantUsers(tenant.id).catch((error) =>
  ...
  const labelClass = dark
    ? 'text-[10px] uppercase text-slate-500 font-bold tracking-wider'
    : 'text-[10px] uppercase text-slate-400 font-bold tracking-wider';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start gap-3">
        <div>
          <h2
            className={
              dark ? 'text-2xl font-bold text-slate-100' : 'text-2xl font-bold text-slate-900'
            }
          >
            Gestão de Clientes (Tenants)
          </h2>
          <p className={dark ? 'text-sm text-slate-400' : 'text-sm text-slate-500'}>
            Cadastro de clientes, configurações básicas e usuários internos.
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={refreshTenants} disabled={busy} className={topBtnClass}>
            <RefreshCw size={14} />
            Atualizar lista
          </button>
          <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 px-1">
        <div className={`border rounded-xl p-3 md:p-4 ${cardClass}`}>
          ...
          <div
            className={
              dark ? 'text-2xl font-bold text-slate-100' : 'text-2xl font-bold text-slate-800'
            }
          >
        ...
        <div className={`border rounded-xl p-3 md:p-4 ${cardClass}`}>
          <div className={labelClass}>Integrados legado</div>
          <div
            className={
              dark ? 'text-2xl font-bold text-emerald-300' : 'text-2xl font-bold text-emerald-700'
            }
          >
            {formatNumber(stats.integrated)}
          </div>
        </div>
        <div className={`border rounded-xl p-3 md:p-4 ${cardClass}`}>
          ...
          <div
            className={
              dark ? 'text-2xl font-bold text-amber-300' : 'text-2xl font-bold text-amber-700'
            }
          >
        ...
        <div className={`border rounded-xl p-3 md:p-4 ${cardClass}`}>
          ...
          <div
            className={
              dark ? 'text-2xl font-bold text-blue-300' : 'text-2xl font-bold text-blue-700'
            }
          >
            {formatNumber(stats.withActive)}
          </div>
        </div>
        <div className={`border rounded-xl p-3 md:p-4 ${cardClass}`}>
          ...
          >
            {formatNumber(stats.activeOrdersTotal)}
          </div>
        </div>
      </div>

      <div
        className={`border rounded-xl p-4 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-3 ${cardClass}`}
      >
        ...
        <select
          ...
          className={
            dark
              ? 'border border-slate-700/60 bg-[#0d1628]/80 text-slate-100 rounded-lg px-3 py-2 text-sm'
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-sm">
            <thead
              className={
                dark
                  ? 'bg-[#0d1628]/95 text-slate-400 text-[11px] uppercase tracking-wide font-semibold'
                  : 'bg-slate-50 text-slate-500 text-[11px] uppercase tracking-wide font-semibold'
              }
            >
              <tr>
            ...
            <tbody className={dark ? 'divide-y divide-slate-700/50' : 'divide-y'}>
              {filtered.map((t) => (
                <tr key={t.id} className={dark ? 'hover:bg-white/[0.03]' : 'hover:bg-slate-50'}>
                  ...
                  <td className="px-6 py-3">
                    <Badge color={t.sgpConfigured ? 'green' : 'orange'}>
                      {t.sgpConfigured ? 'Integrado' : 'Pendente'}
                    </Badge>
                  </td>
                  <td
                    className={
                      dark
                        ? 'px-6 py-3 text-right font-mono text-blue-300 font-bold'
                        : 'px-6 py-3 text-right font-mono text-blue-700 font-bold'
                    }
                  >
                    {t.activeOrders}
                  </td>
                  <td
                    className={
                      dark
                        ? 'px-6 py-3 text-right font-mono text-slate-300'
                        : 'px-6 py-3 text-right font-mono text-slate-700'
                    }
                  >
                    {t.totalOrders}
                  </td>
                  <td className={dark ? 'px-6 py-3 text-slate-400' : 'px-6 py-3 text-slate-600'}>
                  ...
                  >
                    {(tenantUsersByTenant[t.id] || []).length}
                  </td>
                  <td className="px-6 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        className={
                          dark
                            ? 'px-2 py-1 text-xs border border-slate-700/60 rounded bg-[#0d1628]/80 text-slate-200 hover:bg-white/5'
                            : 'px-2 py-1 text-xs border rounded bg-white hover:bg-slate-50'
                        }
                      >
                        Config
                      </button>
                      <button
                        onClick={() => openTenantUsers(t)}
                        className={
                          dark
                            ? 'px-2 py-1 text-xs border border-slate-700/60 rounded bg-[#0d1628]/80 text-slate-200 hover:bg-white/5'
                            : 'px-2 py-1 text-xs border rounded bg-white hover:bg-slate-50'
          ...
          actions={
            <>
              <button
                ...
                className={
                  dark
                    ? 'px-4 py-2 border border-slate-700/60 text-slate-200 rounded-lg text-sm hover:bg-white/5'
              ...
              <button
                ...
                className={
                  dark
                    ? 'px-4 py-2 bg-cyan-500/20 border border-cyan-500/30 text-cyan-100 rounded-lg text-sm disabled:opacity-50 hover:bg-cyan-500/30'
                    : 'px-4 py-2 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50'
                }
              >
                Salvar
              </button>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                value={editingTenant.legalName || ''}
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, legalName: e.target.value } : s))
                }
            ...
            <div>
              ...
              <input
                value={editingTenant.tradeName || ''}
                onChange={(e) =>
                  setEditingTenant((s) =>
                    s ? { ...s, tradeName: e.target.value, name: e.target.value } : s,
                  )
                }
                className={inputClass}
              />
            ...
            <div>
              ...
              <input
                value={editingTenant.taxId || ''}
                onChange={(e) => setEditingTenant((s) => (s ? { ...s, taxId: e.target.value } : s))}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditingTenant((s) => (s ? { ...s, slug: e.target.value } : s))}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, domain: e.target.value } : s))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                value={editingTenant.timezone || 'America/Sao_Paulo'}
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, timezone: e.target.value } : s))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                value={editingTenant.techContactName || ''}
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, techContactName: e.target.value } : s))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                value={editingTenant.techContactEmail || ''}
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, techContactEmail: e.target.value } : s))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                value={editingTenant.techContactPhone || ''}
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, techContactPhone: e.target.value } : s))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingTenant((s) => (s ? { ...s, status: e.target.value } : s))
                }
                className={inputClass}
              >
          ...
          actions={
            <>
              ...
              <button
                ...
                className={
                  dark
                    ? 'px-4 py-2 bg-indigo-500/25 border border-indigo-500/30 text-indigo-100 rounded-lg text-sm disabled:opacity-50 hover:bg-indigo-500/35'
                    : 'px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm disabled:opacity-50'
                }
              >
                Criar
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, legalName: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, tradeName: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTenant((s: TenantForm) => ({ ...s, taxId: e.target.value }))}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTenant((s: TenantForm) => ({ ...s, slug: e.target.value }))}
                className={inputClass}
                placeholder="auto pelo nome fantasia"
              />
            </div>
            <div>
              <label className={labelClass}>Domínio</label>
              <input
                ...
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, domain: e.target.value }))
                }
                className={inputClass}
                placeholder="auto: slug.ajust.local"
              />
            </div>
            <div>
              <label className={labelClass}>Timezone</label>
              <input
                ...
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, timezone: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>Contato Técnico</label>
              <input
                value={newTenant.techContactName}
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, techContactName: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>Email Técnico</label>
              <input
                value={newTenant.techContactEmail}
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, techContactEmail: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>Telefone Técnico</label>
              <input
                value={newTenant.techContactPhone}
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, techContactPhone: e.target.value }))
                }
                className={inputClass}
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewTenant((s: TenantForm) => ({ ...s, status: e.target.value }))
                }
                className={inputClass}
              >
                <option value="ACTIVE">ACTIVE</option>
        ...
        >
          <div className="space-y-4">
            <div className="space-y-2">
              {(tenantUsersByTenant[userTenant.id] || []).map((userItem: TenantUser) => (
                <div
                  ...
                  className={
                    dark
                      ? 'border border-slate-700/60 rounded-lg px-3 py-2 text-sm flex justify-between items-center bg-[#0d1628]/80'
                      : 'border border-slate-200 rounded-lg px-3 py-2 text-sm flex justify-between items-center'
                  }
                >
                  <div>
                    <div
                  ...
                  <Badge color={userItem.active ? 'green' : 'orange'}>
              ...
              {(tenantUsersByTenant[userTenant.id] || []).length === 0 && (
                <div className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>
                  Nenhum usuario carregado para este tenant.
                </div>
              )}
            </div>
            ...
            <div
              className={
                dark
                  ? 'border-t border-slate-700/60 pt-4 grid grid-cols-1 md:grid-cols-2 gap-3'
                  : 'border-t border-slate-100 pt-4 grid grid-cols-1 md:grid-cols-2 gap-3'
              }
            >
              <div>
                ...
                <input
                  value={newUser.name}
                  onChange={(e) =>
                    setNewUser((s: NewTenantUserForm) => ({ ...s, name: e.target.value }))
                  }
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  value={newUser.email}
                  onChange={(e) =>
                    setNewUser((s: NewTenantUserForm) => ({ ...s, email: e.target.value }))
                  }
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewUser((s: NewTenantUserForm) => ({ ...s, password: e.target.value }))
                  }
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewUser((s: NewTenantUserForm) => ({ ...s, roleCode: e.target.value }))
                  }
                  className={inputClass}
                >
                  {availableRoles.map((role) => (
                    <option key={role.code} value={role.code}>
                      {ROLE_CODE_TO_LABEL[role.code] || role.name}
                    </option>
                  ))}
                </select>
              </div>

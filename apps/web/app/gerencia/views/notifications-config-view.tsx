...
export function NotificationsConfigView({
  dark,
  tenantId,
  onToast,
}: {
  ...
  const [form, setForm] = useState<{
    ...
    trigger: NotificationTrigger;
    target: string;
  }>({ name: '', channel: 'EMAIL', trigger: 'ORDER_CREATED', target: '' });
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch(
        `/api/notifications/config?tenantId=${encodeURIComponent(tenantId)}`,
        { cache: 'no-store' },
      );
      ...
      if (res.ok) {
        ...
        const message =
          ...
          typeof (payload as { error?: unknown }).error === 'string'
            ...
            : 'Falha ao carregar configurações de notificação.';
        setLoadError(message);
        setConfigs([]);
      }
    } catch {
      const message = 'Falha ao carregar configurações de notificação.';
      setLoadError(message);
      onToast(message);
      setConfigs([]);
    } finally {
  ...
  const save = async () => {
    if (!form.name.trim() || !form.target.trim()) {
      onToast('Nome e destino são obrigatórios.');
      return;
    }
    setSaving(true);
    try {
      ...
      onToast('Notificação criada!');
      setShowForm(false);
      setForm({ name: '', channel: 'EMAIL', trigger: 'ORDER_CREATED', target: '' });
      load();
    } catch (e: unknown) {
      onToast(e instanceof Error ? e.message : 'Falha ao criar notificação.');
    } finally {
      setSaving(false);
    }
  };

  const toggle = async (cfg: NotificationConfig) => {
    try {
      const res = await fetch(`/api/notifications/config/${cfg.id}?tenantId=${tenantId}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ active: !cfg.active }),
      });
      if (!res.ok) throw new Error('Falha ao atualizar notificação.');
      onToast(`Notificação ${cfg.active ? 'desativada' : 'ativada'}.`);
      load();
    } catch (e: unknown) {
      onToast(e instanceof Error ? e.message : 'Falha ao atualizar notificação.');
    }
  };
  ...
  const remove = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/notifications/config/${id}?tenantId=${tenantId}`, {
        method: 'DELETE',
      });
      ...
      onToast(e instanceof Error ? e.message : 'Falha ao remover notificação.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={cn('text-sm mt-0.5', dark ? 'text-slate-400' : 'text-slate-500')}>
            Configure alertas automáticos por Email, Webhook ou Slack para eventos do NOC.
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-semibold shadow-lg shadow-amber-500/20 transition-colors"
        >
      ...
      <div
        className={cn(
          ...
          dark ? 'bg-slate-800/30 border-slate-700/50' : 'bg-amber-50 border-amber-200',
        )}
      >
        ...
        <div>
          ...
          <p className={cn('text-xs leading-relaxed', dark ? 'text-slate-400' : 'text-slate-600')}>
            Configure um gatilho (ex: &quot;O.S Criada&quot;) + um canal (Email, Webhook ou Slack).
            Toda vez que o evento ocorrer no seu tenant, o Ajust ERP enviará automaticamente a
            notificação para o destino configurado.
          </p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div
            className={cn(
              ...
              dark ? 'bg-slate-800/30 border-slate-700/50' : 'bg-slate-50 border-slate-200',
            )}
          >
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={cn('font-semibold text-sm', dark ? 'text-white' : 'text-slate-800')}
                  >
                    {cfg.name}
                  </p>
                  <p className={cn('text-xs mt-0.5', dark ? 'text-slate-400' : 'text-slate-500')}>
                    <span className="font-medium">{tr?.label || cfg.trigger}</span> →{' '}
                    <span className="font-mono text-[10px] truncate">{cfg.target}</span>
                  </p>
                </div>
          ...
          >
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Slack — O.S Crítica"
                  className={inputCls}
                />
              </div>
              <div>
                  ...
                  {CHANNELS.map((c) => {
                    ...
                    return (
                      <button
                        key={c.value}
                        onClick={() =>
                          setForm({ ...form, channel: c.value as NotificationChannel })
                        }
                        className={cn(
              ...
              <div>
                ...
                <select
                  value={form.trigger}
                  onChange={(e) =>
                    setForm({ ...form, trigger: e.target.value as NotificationTrigger })
                  }
            ...
            >
              ...
              <button
                onClick={save}
                disabled={saving}
                className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-semibold disabled:opacity-60 flex items-center gap-2"
              >

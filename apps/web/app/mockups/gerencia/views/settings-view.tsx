...
export function SettingsViewModule({ role, notify, tenantId, helpers }: SettingsViewModuleProps) {
  ...
  const [newSla, setNewSla] = useState({ priority: 'NORMAL', serviceOrderType: '', hours: 24, isOverride: false });
  ...
  const loadSectors = async () => {
    ...
    setSectorError('');
    try {
      const response = await fetch(`/api/iam/tenants/${encodeURIComponent(tenantId)}/sectors`, { cache: 'no-store' });
      if (!response.ok) {
      ...
      const mapped = Array.isArray(payload)
        ? payload
          ...
          .map((item: any) => ({ id: item.id, name: item.name.trim() }))
  ...
  const saveSlaPolicy = async (policy: any) => {
  ...
  useEffect(() => {
    const loadRoles = async () => {
      ...
      try {
        ...
        if (mapped.length > 0) {
          setRoles(mapped);
          const defaultRole = mapped.find((roleItem: { code: string; name: string }) => roleItem.code === 'analista')?.code || mapped[0].code;
  ...
  const functionRoleOptions = (roles.length ? roles : [{ code: 'analista', name: 'Analista' }]).filter(
    (roleItem: { code: string; name: string }) => roleItem.code === 'analista' || roleItem.code === 'gerente',
  );
  ...
  const roleAsFunctionLabel = (roleCode: string) => (roleCode === 'gerente' ? 'Gestor' : 'Analista');
  ...
  const validateEditFields = () => {
    ...
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(editUser.email.trim())) errs.editEmail = 'E-mail invalido.';
    if (editUser.password && editUser.password.length < 8) errs.editPassword = 'Minimo 8 caracteres.';
  ...
  const createUser = () => {
    ...
    (async () => {
      try {
        ...
        setNewUser((prev) => ({ ...prev, name: '', email: '', password: '', sector: sectorOptions[0] || 'NOC' }));
        setFieldErrors({});
        setModalType(null);
  ...
  const saveSectorEdit = () => {
    ...
    (async () => {
      ...
      setSectorError('');
      try {
        const response = await fetch(`/api/iam/tenants/${encodeURIComponent(tenantId)}/sectors/${encodeURIComponent(sectorEditId)}`, {
          method: 'PATCH',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ name }),
        });
  ...
  const removeSector = (sector: TenantSector) => {
    ...
    (async () => {
      ...
      setSectorError('');
      try {
        const response = await fetch(`/api/iam/tenants/${encodeURIComponent(tenantId)}/sectors/${encodeURIComponent(sector.id)}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className={cn('flex items-center gap-4 border-b pb-6', dark ? 'border-slate-700/60' : 'border-slate-200')}>
        ...
        <div>
          ...
          <p className={cn('mt-1', dark ? 'text-slate-400' : 'text-slate-500')}>Gestão de usuários do tenant.</p>
        </div>
      </div>
        ...
        <div className={sectionCardClass}>
          <div className={sectionHeaderClass}>
            <h3 className={cn('font-bold text-sm uppercase tracking-wide flex items-center', dark ? 'text-slate-200' : 'text-slate-800')}>
              <Settings size={16} className={cn('mr-2', dark ? 'text-cyan-300' : 'text-cyan-600')} /> Setores Operacionais
            </h3>
          </div>
          <div className="p-4 space-y-4">
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <button
                ...
                className={cn('px-4 py-2 text-sm rounded-lg border whitespace-nowrap transition-colors disabled:opacity-60',
            ...
            {sectorError && (
              <div className={cn('text-xs border rounded-lg px-3 py-2',
                dark ? 'border-rose-700/40 bg-rose-900/20 text-rose-300' : 'border-rose-200 bg-rose-50 text-rose-700')}>
                {sectorError}
              </div>
            )}
            <div className={cn('divide-y rounded-lg border', dark ? 'divide-slate-700/60 border-slate-700/60' : 'divide-slate-100 border-slate-200')}>
              {sectorsLoading && (
                <div className="p-4 text-sm text-slate-500">Carregando setores...</div>
              )}
              {!sectorsLoading && sectors.length === 0 && (
                <div className="p-4 text-sm text-slate-500">
                  Nenhum setor customizado cadastrado. O analista usa a lista padrão até você criar setores aqui.
                </div>
              )}
                ...
                <div key={sector.id} className="p-3 flex items-center gap-2">
                  {sectorEditId === sector.id ? (
                    <>
                      ...
                      />
                      <button onClick={saveSectorEdit} disabled={sectorSaving} className={secondaryBtnClass}>
                        Salvar
                      </button>
                      <button onClick={() => { setSectorEditId(null); setSectorEditName(''); }} className={secondaryBtnClass}>
                        Cancelar
                      </button>
                    </>
                  ) : (
                    <>
                      <div className={cn('flex-1 text-sm font-medium', dark ? 'text-slate-200' : 'text-slate-800')}>{sector.name}</div>
                      <button onClick={() => { setSectorEditId(sector.id); setSectorEditName(sector.name); }} className={secondaryBtnClass}>
                        Editar
                      </button>
                      <button onClick={() => removeSector(sector)} className={dangerBtnClass}>
                        Apagar
                      </button>
                    </>
                  )}
          ...
          <div className="p-4 overflow-x-auto">
            {auditLoading ? (
              ...
              <table className="w-full text-sm text-left">
                <thead className={cn('text-xs uppercase', dark ? 'text-slate-400 bg-slate-800/50' : 'text-slate-500 bg-slate-50')}>
                  <tr>
                ...
                <tbody className={cn('divide-y', dark ? 'divide-slate-700/60' : 'divide-slate-200')}>
                  {auditLogs.map((log) => (
                    <tr key={log.id} className={cn('transition-colors', dark ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-50')}>
                      <td className="px-4 py-3 whitespace-nowrap">{new Date(log.createdAt).toLocaleString('pt-BR')}</td>
                      <td className="px-4 py-3"><Badge color="blue">{log.action}</Badge></td>
                      <td className="px-4 py-3">{log.actorUser?.name || 'Sistema'}</td>
                      <td className="px-4 py-3">{log.resourceType} {log.resourceId}</td>
          ...
          <div className="p-4 space-y-4">
            <div className="flex flex-col md:flex-row gap-2 items-end">
              <div className="w-full">
                ...
                <select className={inputBaseClass} value={newSla.priority} onChange={e => setNewSla({...newSla, priority: e.target.value})}>
              ...
              <div className="w-full">
                <label className={labelClass}>Tipo de OS (Opcional)</label>
                <input className={inputBaseClass} placeholder="Ex: ROMPIMENTO" value={newSla.serviceOrderType} onChange={e => setNewSla({...newSla, serviceOrderType: e.target.value.toUpperCase()})} />
              </div>
              <div className="w-full">
                <label className={labelClass}>Horas</label>
                <input type="number" min="1" className={inputBaseClass} value={newSla.hours} onChange={e => setNewSla({...newSla, hours: Number(e.target.value)})} />
              </div>
              <label className="flex items-center gap-2 mb-3 whitespace-nowrap text-sm cursor-pointer">
                <input type="checkbox" checked={newSla.isOverride} onChange={e => setNewSla({...newSla, isOverride: e.target.checked})} />
                Sobrescrever Genéricos
              </label>
              <button 
                onClick={() => saveSlaPolicy({ ...newSla, serviceOrderType: newSla.serviceOrderType || null })}
                disabled={slaSaving}
                className={cn('px-4 py-2 text-sm rounded-lg border whitespace-nowrap transition-colors disabled:opacity-60', dark ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-200 hover:bg-cyan-500/30' : 'bg-blue-600 text-white border-blue-600 hover:bg-blue-700')}
              >
            ...
            <div className={cn('divide-y rounded-lg border', dark ? 'divide-slate-700/60 border-slate-700/60' : 'divide-slate-100 border-slate-200')}>
              {slaLoading ? (
                ...
                <div className="p-4 text-sm text-slate-500">Nenhuma política cadastrada. O sistema usará os padrões do Ajust ERP.</div>
              ) : (
                slaPolicies.map(p => (
                  <div key={p.id} className="p-3 flex items-center justify-between gap-4">
                    <div className="flex gap-4 items-center">
                      <Badge color={p.priority === 'CRITICA' ? 'red' : p.priority === 'ALTA' ? 'orange' : 'blue'}>{p.priority}</Badge>
                      <span className={cn('text-sm font-bold', dark ? 'text-slate-200' : 'text-slate-800')}>{p.hours} horas</span>
                      <span className={cn('text-xs', dark ? 'text-slate-400' : 'text-slate-500')}>{p.serviceOrderType ? `Aplicável apenas em: ${p.serviceOrderType}` : 'Padrão da Prioridade'}</span>
                      {p.isOverride && <Badge color="purple" className="text-[10px]">Override</Badge>}
                    </div>
                    <button onClick={() => saveSlaPolicy({ id: p.id, active: !p.active })} className={secondaryBtnClass}>
      ...
      {modalType === 'user' && (
        ...
        >
          <div className="space-y-4">
            ...
            <div>
              <label className={labelClass}>Função<span className="text-rose-500 ml-0.5">*</span></label>
              <select value={newUser.roleCode} onChange={(e) => setNewUser((s) => ({ ...s, roleCode: e.target.value }))} className={inputBaseClass}>
                {availableFunctionRoleOptions.map((roleItem: { code: string; name: string }) => (
                  <option key={roleItem.code} value={roleItem.code}>{roleAsFunctionLabel(roleItem.code)}</option>
                ))}
              </select>
              {fieldErrors.roleCode && <p className="text-xs text-rose-500 mt-1">{fieldErrors.roleCode}</p>}
            </div>
            <div>
              <label className={labelClass}>Setor<span className="text-rose-500 ml-0.5">*</span></label>
              <select
              ...
              {fieldErrors.sector && <p className="text-xs text-rose-500 mt-1">{fieldErrors.sector}</p>}
            </div>
          </div>
        </Modal>
      )}
          ...
          actions={
            <>
              ...
              <button
                ...
                className={cn('px-4 py-2 text-sm rounded-lg border transition-colors disabled:opacity-60',
                  dark ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-200 hover:bg-cyan-500/30' : 'bg-blue-600 text-white border-blue-600 hover:bg-blue-700')}
              >
        ...
        >
          <div className="space-y-4">
            <div>
              ...
              />
              {fieldErrors.editName && <p className="text-xs text-rose-500 mt-1">{fieldErrors.editName}</p>}
            </div>
            <div>
              <label className={labelClass}>E-mail<span className="text-rose-500 ml-0.5">*</span></label>
              <input
              ...
              />
              {fieldErrors.editEmail && <p className="text-xs text-rose-500 mt-1">{fieldErrors.editEmail}</p>}
            </div>
            <div>
              ...
              />
              {fieldErrors.editPassword && <p className="text-xs text-rose-500 mt-1">{fieldErrors.editPassword}</p>}
            </div>
            <div>
              <label className={labelClass}>Função<span className="text-rose-500 ml-0.5">*</span></label>
              <select
              ...
              {fieldErrors.editRoleCode && <p className="text-xs text-rose-500 mt-1">{fieldErrors.editRoleCode}</p>}
            </div>
            <div>
              <label className={labelClass}>Setor<span className="text-rose-500 ml-0.5">*</span></label>
              <select
              ...
              {fieldErrors.editSector && <p className="text-xs text-rose-500 mt-1">{fieldErrors.editSector}</p>}
            </div>
          </div>
        </Modal>
      )}

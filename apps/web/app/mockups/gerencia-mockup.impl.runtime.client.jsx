...
import { ErpBadge, ErpSidebarItem, ErpDataTable, ErpStatusBadge, ErpEmptyState } from './shared-ui';
import { SidebarUtilityButtons } from './sidebar-utility-buttons';
import { canManageUsers as canManageUsersByRole } from './rbac';
import { AdminDashboardViewModule } from './gerencia/views/admin-dashboard-view';
...
const ProviderDetailDrawer = ({ provider, onClose, onSelectOS }) => {
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-center justify-center p-3 md:p-4 bg-slate-900/75 backdrop-blur-md" onClick={onClose}>
      <div
        onClick={(e) => e.stopPropagation()}
        className={cn(
          'w-full max-w-7xl rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col border',
          dark ? 'bg-[#0b1426] border-slate-700/60' : 'bg-white border-slate-200'
        )}
      >
        <div className={cn('px-4 md:px-6 py-4 border-b flex justify-between items-center shrink-0',
        ...
        <div className="p-4 md:p-6 flex-1 min-h-0 overflow-auto space-y-4 custom-scrollbar">
...
const ProvidersAnalysisView = ({ orders, onSelectOS }) => {
  ...
  const statsGridClass = 'grid grid-cols-2 md:grid-cols-4 gap-3';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className={cn(statsGridClass, 'px-1')}>
        ...
        <div className={metricCardClass}>
          <div className={metricLabelClass}>Média por Provedor / SLA Fora</div>
          <div className={cn('text-2xl font-bold mt-1', dark ? 'text-emerald-300' : 'text-emerald-700')}>
            {providersSummary.avg.toFixed(1)} / {providersSummary.avgLateClosedRate.toFixed(1)}%
          </div>
        </div>
      </div>

      <div className={providersPanelClass}>
        <div className={cn('px-4 md:px-6 py-5 border-b flex flex-col md:flex-row md:items-center justify-between gap-4',
          dark ? 'border-slate-700/60 bg-[#0d1628]/80' : 'border-slate-200 bg-white')}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={cn('font-bold', dark ? 'text-slate-100' : 'text-slate-800')}>Ranking</h3>
              <span className={cn('text-xs', dark ? 'text-slate-400' : 'text-slate-500')}>Criadas na janela</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 w-full lg:w-auto">
            <div className="relative sm:col-span-2">
              <input
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className={cn('pl-9 pr-4 py-2 border rounded-lg text-sm w-full',
                  dark ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200 placeholder:text-slate-500' : 'border-slate-300')}
              />
              <Search className={cn('absolute left-3 top-1/2 -translate-y-1/2', dark ? 'text-slate-500' : 'text-slate-400')} size={14} />
            </div>
            <select value={cityFilter} onChange={(e) => setCityFilter(e.target.value)} className={cn('px-3 py-2 border rounded-lg text-sm', dark ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200' : 'border-slate-300')}>
              {cityOptions.map((city) => <option key={city}>{city}</option>)}
            </select>
            <div className="flex items-center gap-2">
              <input value={minVolume} onChange={(e) => setMinVolume(e.target.value)} placeholder="Min OS" className={cn('w-20 px-2 py-2 border rounded-lg text-sm', dark ? 'bg-[#0d1628]/80 border-slate-700/60 text-slate-200' : 'border-slate-300')} />
              <label className={cn('text-xs inline-flex items-center gap-1 whitespace-nowrap', dark ? 'text-slate-300' : 'text-slate-600')}>
        ...
        <div className={cn('px-4 md:px-6 border-b flex flex-wrap items-center gap-2 py-3',
          ...
          <div className={cn('ml-auto text-[11px] font-medium self-center pr-1', dark ? 'text-slate-400' : 'text-slate-500')}>
            Criticas Ativas: {formatNumber(providersSummary.criticalActive)} • Vencem Hoje: {formatNumber(providersSummary.dueToday)}
          </div>
        </div>
...
const SLAView = ({ orders, onSelectOS }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <h2 className={cn('text-2xl font-bold', dark ? 'text-slate-100' : 'text-slate-900')}>SLA & Performance</h2>
        <WindowControls period={period} setPeriod={setPeriod} customRange={customRange} setCustomRange={setCustomRange} />
      </div>
...
const AnalystDashboardView = ({ orders, onSelectOS, loading }) => {
  ...
  const renderView = () => {
    switch (currentView) {
      case 'analyst_dashboard':
        return <AnalystDashboardView orders={orders} onSelectOS={setSelectedOS} loading={initialLoading} />;
      ...
      case 'tenants':
        return (
          <TenantManagementView
            ...
            orders={orders}
            notify={notify}
            onReloadTenants={() => loadRemoteData(tenantId, userRole)}
          />
        );
